...
def accio(valor):
    finestra.geometry(f"{valor}x{valor}")
...
b_lliscan1 = tk.Scale(from_=0, to=100, command=lambda valor: accio(int(valor)))
b_lliscan1.grid(padx=5, pady=5, column=1, rowspan=4, sticky="ne")

b_lliscan2 = tk.Scale(orient="horizontal", from_=0, to=100, command=lambda valor: accio(int(valor)))
